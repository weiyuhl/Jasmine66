...
### 10. 设置页面重构 & 配置迁移
- [x] 将设置页改为入口列表设计（"基本设置"与"开源许可"）
- [x] "基本设置"子页面聚合主题、动态色彩和深色模式控制
- [x] 将 LLM `ChatProviderRepository` 下沉到 `core:data` 解耦 AI 依赖
- [x] 在设置菜单增加 "大模型配置" 入口（平移原来的 Bottom Sheet 表单）
- [x] 在聊天界面当未配置时给出文字提示

### 11. 全局 UI 极简风重构
- [x] 弃用原有的默认紫色/紫红主题，全面替换为纯正的现代黑白灰（Monochrome）极简配色。
- [x] 同时保留了主界面输入框与空状态（Lottie）组件的原始代码逻辑，仅令其色调自然融入极简体系。
